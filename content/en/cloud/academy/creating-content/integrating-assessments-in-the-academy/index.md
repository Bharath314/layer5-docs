...
the overall `pass_percentage` for the assessment. Here is how the system
...
3.  **Pass/Fail Status**: The system calculates the final percentage using the
    ...
    is greater than or equal to the `pass_percentage` you set, the assessment is
    marked as "Passed".
...
- **Short Answer Questions**: The learner's input is compared against the
  `correct_answer` field. The comparison is **case-insensitive**, and
  leading/trailing whitespace is ignored to avoid penalizing minor typing
  variations.
